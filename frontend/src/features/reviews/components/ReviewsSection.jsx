  ...
  return (
    <div className="rounded-2xl border border-brand-100 bg-brand-50/30 p-5 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onCancel} className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors">
      ...
      {/* Reviewer type */}
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">You are a</p>
      ...
      {/* Ratings */}
      <div>
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Rate each category</p>
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 resize-none bg-white"
        />
  ...
  return (
    <div className="p-4 rounded-xl border border-slate-100 bg-white">
        ...
        <div className="mt-3 ml-3 border-l-2 border-brand-200 pl-3">
          <div className="flex items-center gap-2 mb-1.5">
            {ownerInfo?.avatarUrl ? (
              ...
              <div className="w-6 h-6 rounded-full bg-brand-100 text-brand-700 text-[11px] font-bold flex items-center justify-center shrink-0">
                {(ownerInfo?.name ?? 'O')[0].toUpperCase()}
              </div>
            )}
            <span className="text-[11px] font-semibold text-brand-700">{ownerInfo?.name ?? 'Owner'}</span>
            <span className="text-[11px] text-slate-500">· Owner response</span>
          </div>
          <p className="text-xs text-slate-700 leading-relaxed">{review.ownerResponse}</p>
          {isOwner && (
            <button onClick={() => { setDraft(review.ownerResponse ?? ''); setReplyOpen(true) }} className="mt-1.5 text-[11px] font-semibold text-brand-600 hover:underline">
              Edit response
            </button>
          )}
        </div>
      ...
      {isOwner && !review.ownerResponse && !replyOpen && (
        <button onClick={() => setReplyOpen(true)} className="mt-3 flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-brand-600 transition-colors">
      ...
      {isOwner && replyOpen && (
        <div className="mt-3 space-y-2">
          <textarea
            ...
            className="w-full border border-slate-200 rounded-xl px-3 py-2.5 text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 resize-none"
          />
          <div className="flex gap-2">
            <button onClick={() => replyMutation.mutate(draft.trim())} disabled={replyMutation.isPending || !draft.trim()} className="min-h-[44px] flex-1 py-3 rounded-xl bg-brand-600 text-white text-xs font-semibold hover:bg-brand-700 disabled:opacity-50 transition-colors">
              {replyMutation.isPending ? 'Saving…' : 'Post response'}
            </button>
            {review.ownerResponse && (
              <button onClick={() => replyMutation.mutate('')} disabled={replyMutation.isPending} className="min-h-[44px] px-3 py-3 rounded-xl bg-red-50 text-red-600 text-xs font-semibold hover:bg-red-100 disabled:opacity-50 transition-colors">
                Remove
              </button>
            )}
            <button onClick={() => setReplyOpen(false)} className="min-h-[44px] px-3 py-3 rounded-xl bg-slate-100 text-slate-500 text-xs font-semibold hover:bg-slate-200 transition-colors">
              Cancel
            </button>
          </div>
        </div>
