    ...
    <div className="flex items-center justify-between py-2.5 border-b border-slate-50 last:border-0">
      ...
      <div className="flex gap-0.5 shrink-0">
        {[1, 2, 3, 4, 5].map(n => (
          ...
          >
            {/* slate-200 left the unselected star at 1.16:1 on white — an
                interactive control the user could barely see. slate-500 (4.71:1)
                matches the mobile StarPicker; slate-400 is only 2.49:1 and
                fails even the 3:1 bar for a control. */}
            <Star className={`w-6 h-6 transition-colors ${n <= value ? 'text-amber-400' : 'text-slate-500 hover:text-amber-200'}`} fill="currentColor" stroke="none" />
          </button>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="rounded-2xl border border-brand-100 bg-brand-50/30 p-5 space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={onCancel} aria-label="Cancel review" className="w-7 h-7 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-500 transition-colors">
