...
export default function SourcesFooter({ sources, computedAt }) {
  ...
  return (
    <details className="mt-2 group">
      <summary className="text-[11px] text-slate-500 cursor-pointer list-none hover:text-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 rounded">
        Sources &amp; freshness
      ...
      <div className="mt-1.5 space-y-0.5">
        {sources?.map((s) => (
          <p key={s.name} className="text-[11px] text-slate-500">
            {s.name}
            {s.license && <span className="text-slate-500"> · {s.license}</span>}
            {s.fetchedAt && <span className="text-slate-500"> · {s.fetchedAt}</span>}
          </p>
        ))}
        {computedAt && (
