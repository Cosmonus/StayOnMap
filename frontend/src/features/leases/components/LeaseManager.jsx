      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full min-h-[44px] border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30"
          />
        ...
        <div>
          ...
          <textarea value={form.ownerNote} onChange={e => setForm(f => ({ ...f, ownerNote: e.target.value }))}
            ...
            className="w-full min-h-[44px] border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30 resize-none" />
        </div>

        <button type="submit" disabled={isPending}
          className="w-full min-h-[44px] py-3 bg-[#111111] text-white text-sm font-semibold rounded-xl hover:opacity-90 disabled:opacity-50 transition-opacity">
      ...
      {showConfirm ? (
        <div className="px-5 py-4 space-y-3 bg-slate-50">
          <textarea
            ...
            className="w-full min-h-[44px] border border-slate-200 rounded-xl px-3 py-2.5 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30 resize-none bg-white"
          />
          <div className="flex gap-2">
            <button onClick={() => doAction(showConfirm)} disabled={isPending}
              className={`flex-1 min-h-[44px] py-3 text-sm font-semibold rounded-xl text-white transition-opacity disabled:opacity-50 ${showConfirm === 'sign' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-500 hover:bg-red-600'}`}>
              {isPending ? 'Processing…' : showConfirm === 'sign' ? 'Confirm & sign' : showConfirm === 'reject' ? 'Confirm rejection' : 'Confirm termination'}
            </button>
            <button onClick={() => setShowConfirm(null)}
              className="min-h-[44px] px-4 py-3 text-sm font-semibold rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-100 transition-colors">
              Cancel
            </button>
          </div>
        </div>
        ...
        <div className="px-5 py-3 flex gap-2">
          {iAmTenant && lease.status === 'OFFERED' && (
            <>
              <button onClick={() => setShowConfirm('sign')}
                className="flex-1 min-h-[44px] py-3 text-sm font-semibold text-white bg-green-600 hover:bg-green-700 rounded-xl transition-colors">
                Sign lease
              </button>
              <button onClick={() => setShowConfirm('reject')}
                className="flex-1 min-h-[44px] py-3 text-sm font-semibold text-red-600 border border-red-200 hover:bg-red-50 rounded-xl transition-colors">
                Reject
              </button>
            </>
          )}
          ...
          {iAmOwner && lease.status === 'ACTIVE' && (
            <button onClick={() => setShowConfirm('terminate')}
              className="min-h-[44px] py-3 px-4 text-sm font-semibold text-red-600 border border-red-200 hover:bg-red-50 rounded-xl transition-colors">
              Terminate
            </button>
          )}
        </div>
