    ...
    <div className="mt-6 max-w-xs">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm">
        <div className="flex items-center justify-between mb-3">
          <button type="button" onClick={() => setCursor((c) => c.month === 0 ? { year: c.year - 1, month: 11 } : { ...c, month: c.month - 1 })} className="text-slate-400 hover:text-slate-700 px-1">‹</button>
          <p className="text-sm font-bold text-slate-900">{monthLabel}</p>
          <button type="button" onClick={() => setCursor((c) => c.month === 11 ? { year: c.year + 1, month: 0 } : { ...c, month: c.month + 1 })} className="text-slate-400 hover:text-slate-700 px-1">›</button>
        </div>
            ...
            return (
              <button
                ...
                className={`aspect-square rounded-lg text-xs font-mono flex items-center justify-center transition-colors ${
                  blocked ? 'bg-slate-100 text-slate-400 line-through' : 'text-slate-700 hover:bg-brand-50'
                }`}
              >
                {day}
              </button>
      ...
      <p className="text-xs text-slate-400 mt-2.5">Tap a date to block it. Strikethrough = blocked for guests.</p>
    </div>
  )
}
