...
export async function adminListReviews({ status, page = 1, limit = 20 }) {
  ...
  const [reviews, total] = await Promise.all([
    prisma.communityReview.findMany({ where, skip, take: limit, orderBy: { createdAt: 'desc' }, include: { property: { select: { id: true, title: true } }, reviewer: { select: { id: true, name: true } } } }),
    prisma.communityReview.count({ where }),
  ])
  return { reviews, total, page, limit }
}

export async function adminSetReviewStatus(reviewId, status) {
  const review = await prisma.communityReview.update({ where: { id: reviewId }, data: { status } })
  await recalculateTrustScore(review.propertyId)
  return review
}
