 ...
 * words live in features/admin/components/support/supportVocab.js (web only —
 * admins are platform operators, so mobile has no admin vocabulary at all).
 */

// TONES ARE THEME COLOURS HERE, not Tailwind classes — the words are mirrored
// from web, the styling cannot be. Same three roles: muted for 'nothing is
// asked of you', brand for 'we are on it', amber for the one status that needs
// the reader to DO something.
export const STATUS_COPY = {
  OPEN:              { label: 'Sent — waiting for us to look', tone: 'muted' },
  TRIAGED:           { label: 'We have read it', tone: 'muted' },
  IN_PROGRESS:       { label: 'We are working on it', tone: 'brand' },
  // The one status that asks something of the reader, so it is the one that
  // gets a colour they will notice.
  WAITING_FOR_USER:  { label: 'We need something from you', tone: 'attention' },
  WAITING_FOR_OWNER: { label: 'Waiting on the owner', tone: 'muted' },
  // Never "escalated" — an internal word that reads as either alarming or
  // meaningless depending on who you are.
  ESCALATED:         { label: 'With our specialist team', tone: 'muted' },
