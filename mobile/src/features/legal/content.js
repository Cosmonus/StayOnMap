  ...
  sections: [
    ...
    },
    {
      ...
      blocks: [
        ...
        { lead: 'Product analytics — Google Analytics.', p: 'On our website we also use Google Analytics 4 to understand how people arrive at the Platform and which pages they read. It sets cookies in the browser and sends Google the IP address, the pages viewed, and general device information. See Section 8.' },
        { lead: 'The mobile app is different, and narrower.', p: 'It contains no Google Analytics code and sets no cookies. Instead our own servers pass on the five funnel steps described above — and nothing else. Google receives the step, the city, the listing id, and the random session identifier; because the message comes from our server rather than from your phone, it carries no IP address and no device information.' },
    ...
    },
    {
      ...
      blocks: [
        ...
        { ul: [
          ...
          { lead: 'Google Analytics', text: '(Google LLC) — measures how the Platform is used. From the website it receives the IP address, the pages viewed, device and browser information, and a random identifier stored in cookies. From the mobile app it receives only the five funnel steps, forwarded by our servers, with no IP address or device information. In neither case does it receive your name, email, phone number, or the contents of your messages.' },
    ...
    },
    {
      ...
      blocks: [
        ...
        { lead: 'Google Analytics does set cookies, on the website only.', p: 'Named _ga and _ga_<id>, they hold a randomly generated number that lets Google Analytics tell one browser from another and group requests into a single visit. They last up to two years and are readable only by Google Analytics. We do not use them to identify you by name, and we run no advertising cookies and no ad-network tags today — if that changes, Section 4 says how you will hear about it first. The mobile app sets no cookies and contains no Google Analytics.' },
        { lead: 'Our own analytics uses no cookie at all, and neither does the mobile app.', p: 'The session identifier described in Section 2 lasts for one run of the app (one browser tab on the website), so it cannot follow you between visits or to any other app or website.' },
        { lead: 'Refusing them.', p: 'On the website, use your browser’s cookie controls or Google’s opt-out browser add-on at tools.google.com/dlpage/gaoptout. Nothing about how the Platform works for you changes if you do — every feature stays available.' },
      ],
    },
    {
